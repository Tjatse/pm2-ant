...
Monitor.ACCEPT_KEYS = ['pm2', 'refresh', 'statsd', 'node', 'log'];
...
Monitor.prototype.run = function () {
  ...
  this.logger && this.logger.info('Already running.');
};

/**
 * Set the logger middleware.
 * @param {[type]} logger Log
 */
Monitor.prototype.setLogger = function(logger){
  this.logger = logger;
};
...
Monitor.prototype.quit = function () {
  this.logger && this.logger.debug('Closing pm2 pub emitter socket.');
  this.pm2Sock && this.pm2Sock.close();
  this.logger && this.logger.debug('Closing statsd dgram socket.');
...
Monitor.prototype._observePM2 = function (pm2Daemon, sock, name) {
  this.pm2Sock = pm.sub(pm2Daemon, function (e) {
    ...
    this.logger && this.logger.debug('Sent', data.split(/\n/).join(' '));
  });
};
