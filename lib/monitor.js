...
Monitor.prototype._init = function (options) {
  ...
  try {
    ...
    debug('Can not load PM2 config, the file "' + pm2ConfPath + '" does not exist or empty, fallback to pm2-ant.ini');
    options.pm2Conf = {
...
Monitor.prototype.quit = function () {
  debug('close pm2 pub emitter.');
  this.pm2Sock && this.pm2Sock.close();
  debug('close statsd socket.');
